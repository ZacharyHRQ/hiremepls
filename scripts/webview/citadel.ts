// Runs under `bun`, not Node/tsx — Citadel's careers board is a client-rendered
...
const BASE_URL = "https://www.citadel.com/careers/open-opportunities/";
const MAX_PAGES = Number(process.env.WEBVIEW_MAX_PAGES ?? 8);
...
async function main() {
  ...
  try {
    for (let page = 1; page <= MAX_PAGES; page++) {
      ...
      console.error(`citadel: page ${page} — ${cards.length} card(s)`);
